...
#define VAR_SHOW_LIMIT_LABEL 20
...
-(void)layoutSubviews
{
    ...
    if(textH == 0) {
        ...
        self.scrollView.frame = CGRectMake(0, self.scrollView.frame.origin.y, self.scrollView.frame.size.width, self.scrollView.frame.size.height-textH);
...
-(void)sendFinished:(NSString*)text {
    ...
    CGFloat left = pad;
    CGFloat tpad = 2;
    self.messageBarView.frame = CGRectMake(0, self.scrollView.frame.origin.y+self.scrollView.frame.size.height, self.frame.size.width, self.normalHeight);
    self.backView.frame = CGRectMake(left, pad, self.messageBarView.frame.size.width-(left+btnWidth+pad), self.messageBarView.frame.size.height-(pad*2));
    self.textView.frame = CGRectMake(tpad, 0, self.backView.frame.size.width-(tpad*2), self.backView.frame.size.height);
}
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(text.length > 0)
    {
        ...
        if(self.charLimit > 0)
        {
            if(text.length > self.charLimit)
                enable = NO;
            if (text.length + VAR_SHOW_LIMIT_LABEL > self.charLimit) {
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if([object isKindOfClass:[UITextView class]])
    {
        ...
        UIView *view  = object;
        CGFloat h = view.frame.origin.y; // CHANGE
        if(h > self.maxScrollHeight+self.messageBarView.frame.size.height) {
            h = self.maxScrollHeight + self.scrollView.frame.origin.y;
        } else {
            h -= self.messageBarView.frame.size.height;
        }
        CGRect frame = self.scrollView.frame;
        frame.size.height = h - self.scrollView.frame.origin.y;
